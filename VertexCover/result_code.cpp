    ...
    int main() {
    freopen("results.txt", "w", stdout); // Redirect output to results.txt

    cout << "Testing Vertex Cover on Random Graphs...\n\n";

    for (int vertices = 4; vertices <= 15; vertices++) { // Skip trivial sizes
        double density = 0.3 + (vertices % 5) * 0.1; // Vary density a bit
        cout << "==============================\n";
        cout << "Graph with " << vertices << " vertices, density " << fixed << setprecision(2) << density << "\n";
        cout << "==============================\n";
        ...
        vector<int> brute = VC.Bruteforce();
        vector<int> approx = VC.ApproxAlgo();
        vector<int> anneal = VC.Simulated_Annealing();

        cout << "Brute Force Cover: ";
        for (int x : brute) cout << x << " ";
        cout << "\n";

        cout << "Approximate Cover: ";
        for (int x : approx) cout << x << " ";
        cout << "\n";

        cout << "Simulated Annealing Cover: ";
        for (int x : anneal) cout << x << " ";
        cout << "\n";

        cout << "| Algorithm           | Size of Cover |\n";
        cout << "|---------------------|---------------|\n";
        cout << "| Brute Force         | " << brute.size() << "             |\n";
        cout << "| Approximation       | " << approx.size() << "             |\n";
        cout << "| Simulated Annealing | " << anneal.size() << "             |\n";
        cout << "\n\n";
    }
